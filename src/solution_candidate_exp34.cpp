...
static const int ASSIGN_BLOCK = 10;
static inline int pickRemote() {
    if (cfg.wC >= 0.5 && cfg.K >= 4 && cfg.wTp < 0.5) {
        return (curRid / ASSIGN_BLOCK) % cfg.K;
    }
    int best = 0;
...
static void scheduleFrame(double now) {
    ...
    for (int c = 0; c < cfg.K; c++) {
        ...
        int type = -1; double best = -1e18;
        if (!readyPProc[c].empty()) { double u = urgency(now - readyPProc[c].begin()->first, cfg.slo1, 1) + pProcBias; if (u > best) { best = u; type = 0; } }
